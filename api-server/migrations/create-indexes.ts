...
async function createIndexes() {
	try {
		...
		await User.collection.createIndex(
			{ stripeCustomerId: 1 },
			{ sparse: true, name: 'stripe_customer_lookup' }
		);
		...
		await Project.collection.createIndex(
			{ subdomain: 1 },
			{ unique: true, name: 'subdomain_unique' }
		);
		await Project.collection.createIndex(
			{ user: 1, createdAt: -1 },
		...
		await Deployment.collection.createIndex(
			{ project: 1, createdAt: -1 },
			{ name: 'project_deployments_by_date' }
		);
		await Deployment.collection.createIndex(
			{ user: 1, createdAt: -1 },
			{ name: 'user_deployments_by_date' }
		);
